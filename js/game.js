...
const NEWSVENDOR_PROFILE_STORAGE_KEY = "course_tutorial_platform_newsvendor_profile";
...
function chooseNewsvendorProfile(profiles, previousProfileId, random = Math.random) {
  if (!Array.isArray(profiles) || profiles.length === 0) {
    throw new Error("At least one Newsvendor profile is required.");
  }
  const eligibleProfiles = profiles.filter(
    profile => profile.id !== previousProfileId
  );
  const profilePool = eligibleProfiles.length > 0
    ? eligibleProfiles
    : profiles;
  return profilePool[Math.floor(random() * profilePool.length)];
}
...
function selectNewsvendorProfile() {
  ...
  const selectedProfile = chooseNewsvendorProfile(
    NEWSVENDOR_PROFILES,
    previousProfileId
  );
...
function standardNormalCdf(value) {
  ...
  // Integrate the standard Normal density from zero to |value| with
  // Simpson's rule. This direct numerical implementation keeps the public
  // template independent of third-party inverse-CDF approximation code.
  const upper = Math.min(Math.abs(value), 8);
  const steps = 160;
  const width = upper / steps;
  let weightedDensity = standardNormalDensity(0) + standardNormalDensity(upper);

  for (let index = 1; index < steps; index += 1) {
    weightedDensity += (index % 2 === 0 ? 2 : 4) * standardNormalDensity(index * width);
  }

  const areaFromZero = weightedDensity * width / 3;
  return value > 0 ? 0.5 + areaFromZero : 0.5 - areaFromZero;
}
...
function showNewsvendorDebrief() {
  ...
  document.querySelectorAll("[data-brief-field].brief-changed").forEach(item => {
    item.classList.remove("brief-changed");
  });
